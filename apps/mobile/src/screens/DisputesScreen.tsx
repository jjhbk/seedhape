...
export default function DisputesScreen() {
  ...
  const pending = disputes.filter(d => d.resolution === 'PENDING');
    ...
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        ...
        <View style={[
          ...
          <Text style={[
            styles.badgeText,
            item.resolution === 'PENDING' ? { color: '#92400e' } :
            item.resolution === 'APPROVED' ? { color: '#166534' } : { color: '#991b1b' },
          ]}>
            {item.resolution}
          </Text>
        </View>
      ...
      {item.resolutionNote && (
        <Text style={styles.note}>{item.resolutionNote}</Text>
      )}
      ...
      {item.resolution === 'PENDING' && (
        <View style={styles.actions}>
          ...
          <TouchableOpacity
            ...
            onPress={() => resolve(item.id, 'APPROVED')}
          >

...
export default function TransactionsScreen() {
  ...
  async function loadPage(p: number, reset = false) {
    ...
    const data = await getTransactions(token, p);
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListFooterComponent={
          loading ? <ActivityIndicator style={{ padding: 16 }} color="#16a34a" /> : null
        }
      />
            ...
            {selected && (
              <ScrollView>
                ...
                {selected.originalAmount !== selected.amount && (
                  <DetailRow label="Original Amount" value={`₹${paiseToRupees(selected.originalAmount)}`} />
                )}
